...
// ─── DESIGN TOKENS ────────────────────────────
// Anchor color is KPMG blue #00338D; #83BD41 is the secondary green accent.
const C={
  primary:"#00338D",   // KPMG blue — headings, primary actions, brand surfaces
  primaryDk:"#001F5A", // darker navy for gradients
  green:"#83BD41",     // KPMG secondary accent
  ink:"#1F2937",       // primary body text
  muted:"#6B7280",     // secondary text
  faint:"#9CA3AF",     // captions / de-emphasized text
...
function ChatPanel({onClose,initiative,role,program,minimal}){
  ...
  return<div style={style}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:8}}>
      {msgs.length===0&&<div><p style={{fontSize:11,color:"#9CA3AF",marginBottom:8}}>Suggested queries</p>{queries.map((q,idx)=><button key={idx} onClick={()=>send(q)} style={{width:"100%",textAlign:"left",background:"#F4F7FB",border:"1px solid #E5E7EB",borderRadius:7,padding:"7px 10px",fontSize:11,color:"#374151",cursor:"pointer",marginBottom:5,lineHeight:1.4}}>{q}</button>)}</div>}
    ...
    <div style={{background:"white",borderRadius:12,width:"100%",maxWidth:860,maxHeight:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 24px 64px rgba(0,0,60,0.3)"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:24}}>
        {mode==="idle"&&<div style={{padding:"32px 24px"}}>
          ...
          <p style={{fontSize:11,color:"#9CA3AF",marginTop:10}}>AI will use the programme context and your direction to generate consulting-quality content</p>
        </div>}
            ...
            <div style={{background:"#FFF7ED",padding:"10px 16px",display:"flex",gap:16,borderTop:"1px solid #FED7AA"}}>
              {[["Total Tasks","57"],["On Critical Path","10"],["Active Blockers","1"],["Float","0 days"],["Go-Live","Q3 FY29"]].map(([l,v])=>(
                <div key={l} style={{textAlign:"center"}}>
                  <p style={{fontSize:10,color:"#9CA3AF",textTransform:"uppercase",fontWeight:700,marginBottom:2}}>{l}</p>
          ...
          {PHASE_META.map((ph,idx)=>{
            ...
            return<div key={idx} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",position:"relative"}}>
              ...
              <div style={{textAlign:"center",marginTop:7,paddingRight:idx<5?6:0}}>
                ...
                <p style={{fontSize:10,color:"#9CA3AF",lineHeight:1.3,marginBottom:2}}>{ph.sub}</p>
                <p style={{fontSize:10,fontWeight:700,color:done?"#059669":cur?"#00338D":"#D1D5DB"}}>{done?"Complete":cur?"In Progress":"Upcoming"}</p>
              </div>
            </div>;})}
        </div>
      </div>
          ...
          {ROADMAP_PROJECTS.map((proj,idx)=>{
            ...
            return<div key={proj.id} style={{display:"grid",gridTemplateColumns:"120px 1fr",borderTop:"1px solid #F3F4F6"}} onClick={()=>onSelect&&onSelect(init)}>
              <div style={{padding:"4px 6px",borderRight:"1px solid #F3F4F6",cursor:"pointer"}}>
                <p style={{fontSize:10,fontWeight:700,color:"#1F2937",lineHeight:1.3,textAlign:"left"}}>{proj.name}</p>
              </div>
...
function TSATab({program,onSelect}){
  ...
  const qO={"Q3 FY27":1,"Q4 FY27":2,"Q1 FY28":3,"Q2 FY28":4,"Q3 FY28":5,"Q4 FY28":6,"Q1 FY29":7};
      ...
      {filtered.map((init,idx)=>{
        ...
        return<div key={init.id} onClick={()=>onSelect(init)}
          ...
          <div>
            ...
            <p style={{fontSize:10,color:"#9CA3AF"}}>{init.prog}% progress</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p style={{fontSize:10,color:"#9CA3AF"}}>vs ₹{ptd}Cr PTD</p>
          </div>
        </div>;
      })}
    </div>
    ...
    {Object.entries(CATS).map(([cat,{color,light,border}])=>{
      ...
      return<div key={cat} style={{marginBottom:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:10,background:"white",borderRadius:8,padding:"10px 12px",boxShadow:"0 1px 4px rgba(0,0,0,0.05)",border:`1px solid ${border}`}}>
          <div>
            <p style={{fontSize:10,color:"#9CA3AF",fontWeight:700,textTransform:"uppercase",marginBottom:2}}>Budget</p>
            <p style={{fontSize:12,fontWeight:700,color:color}}>₹{totalSpent}Cr / ₹{totalBudget}Cr</p>
            <p style={{fontSize:10,color:"#9CA3AF"}}>{Math.round(totalSpent/totalBudget*100)||0}% utilised</p>
          </div>
          <div>
            <p style={{fontSize:10,color:"#9CA3AF",fontWeight:700,textTransform:"uppercase",marginBottom:2}}>Variance vs PTD</p>
            <p style={{fontSize:12,fontWeight:700,color:variance>=0?"#059669":"#DC2626"}}>₹{Math.abs(variance)}Cr {variance>=0?"under":"over"}</p>
            <p style={{fontSize:10,color:"#9CA3AF"}}>PTD ₹{ptd}Cr</p>
          </div>
          <div>
            <p style={{fontSize:10,color:"#9CA3AF",fontWeight:700,textTransform:"uppercase",marginBottom:2}}>RAG Status</p>
          ...
          <div>
            <p style={{fontSize:10,color:"#9CA3AF",fontWeight:700,textTransform:"uppercase",marginBottom:2}}>Schedule</p>
            <p style={{fontSize:12,fontWeight:700,color:avgProg>15?"#059669":"#D97706"}}>{avgProg>15?"On Track":"Early Stage"}</p>
            <p style={{fontSize:10,color:"#9CA3AF"}}>{avgProg}% complete</p>
          </div>
        </div>
        {/* Initiative cards */}
